...
namespace Cozyheim.UpgradeUnlimited
{
    ...
    internal class Main : BaseUnityPlugin
    {
        ...
        internal static ConfigEntry<int> itemMaxLevel;
        ...
        void Awake()
        {
            ...
            modEnabled = CreateConfigEntry("General", "ModEnabled", true, "Enable this mod", true); // false = non-synced (client)
            debugEnabled = CreateConfigEntry("General", "DebugEnabled", true, "Display debug messages in the console", false); // true = synced (server)
            itemMaxLevel = CreateConfigEntry("Settings", "itemMaxLevel", 100, "The maximum upgrade quality for all items", true); // true = synced (server)
